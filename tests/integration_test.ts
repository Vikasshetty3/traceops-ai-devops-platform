/**
 * Integration Test Suite for Requirement Traceable DevOps Platform
 * Validates the complete pipeline:
 * Requirement -> SLO -> Metrics -> Incident -> ML -> Gemini RCA -> Experiment -> DevOps -> Traceability
 */
...
async function runTests() {
  ...
  await test("Requirement Creation & Retrieval", async () => {
    ...
    const reqData = (await getRes.json()) as any;
  ...
  await test("SLO Creation Linked to Requirement", async () => {
    ...
    const sloData = (await getRes.json()) as any;
  ...
  await test("Gemini RCA Analysis & MongoDB Persistence", async () => {
    ...
    const rcaData = (await rcaRes.json()) as any;
